...
class PerceptionTestV2:
    ...
    def load_mask_for_marker(self, marker):
        """Load mask for a marker with proper error handling and debug output."""
    ...
    def trigger_marker(self, marker_idx):
        """Trigger a specific marker with comprehensive debug output."""
    ...
    def process_next_marker_if_ready(self):
        """Check if we should trigger the next marker. This ensures sequential processing."""
        # Only check for markers if we're not currently waiting for user input
        if self.waiting_for_click or self.current_marker is not None:
            return
        
        # Check if there's a next marker to process
        ...
        # If current time has reached or passed this marker's time, trigger it
        if self.time_sec >= marker_time:
            ...
            if self.trigger_marker(self.next_marker_idx):
                # Don't increment next_marker_idx here - wait until user answers
                pass
        else:
            # Debug: Show what we're waiting for
            if self.next_marker_idx == 0:  # Only print this occasionally to avoid spam
    ...
    def advance_to_next_marker(self):
        """Move to the next marker after the current one is answered."""
    ...
    def jump_to_marker(self, idx):
        """Jump to a specific marker (for manual navigation)."""
    ...
    def run(self):
        ...
        while True:
            ...
            # Draw mask overlay if available and toggled on
            if self.mask is not None and self.mask_overlay_on and self.waiting_for_click:
                ...
                mask_rgb[self.mask == 255] = [0, 0, 255]  # Red overlay for mask areas
            ...
            if key == ord('q'):
                ...
                # Jump to next marker manually
                if self.next_marker_idx < len(self.markers):
                    self.jump_to_marker(self.next_marker_idx)
            elif key == ord('p'):
                # Jump to previous marker manually
