"""
perception_test_official.py
--------------------------
Official test mode for endoscopy video analysis.
- Strict, linear test environment for formal assessment
- Features: click-based questions, multiple choice questions, polyp reports
- NO practice features: no skipping, no overlays, no debug mode, no navigation shortcuts
- Linear progression only - must complete each question in sequence
- Results are logged and can be exported to CSV

Official Test Mode Restrictions:
- No N/P navigation (must complete questions in order)
- No mask overlays (O key disabled)
- No debug mode (D key disabled)
- No jumping between markers (J key disabled)
- No review screen during test (R key disabled)
...
- Official mode enforces strict test conditions
"""

# This file is a copy of perception_test_v2.py, modified for Official Test Mode.
# Practice features (skipping, overlays, debug, etc.) are disabled.
# Only linear progression is allowed. Review/export features are kept.
# UI text updated to reflect Official Test Mode.
import cv2
import numpy as np
import time
import csv
...
class PerceptionTestV2:
    """
    Official test mode perception test class.
    Handles video playback, question presentation, user interaction, and scoring.
    Enforces strict test conditions with no practice features.
    """
    def __init__(self):
        self.window_name = "Perception Test - Official Test Mode"
        ...
        # Official test mode: no skipping, no debug, no overlays
        self.testing_mode = False  # OFFICIAL MODE: All practice features disabled
    ...
    def draw_sidebar(self, frame):
        ...
        y = overlay_y + 30
        x = overlay_x + 20
        cv2.putText(frame, "Perception Test - Official Test Mode", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
        y += 40
...
def run_perception_test():
    """
    Entry point for running the official test mode perception test.
    
    Launches the strict test environment with all practice features disabled:
    - Linear progression only (no skipping)
    - No debug mode or overlays
    - No navigation shortcuts
    - Must complete questions in sequence
    
    The test loads video and marker data, then runs the main test loop
    with official test restrictions enforced.
    """
